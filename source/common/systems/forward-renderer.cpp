...
namespace our {
    ...
    void ForwardRenderer::initialize(glm::ivec2 windowSize,
        ...
        if (config.contains("sky")) {
            ...
            skyShader->attach("assets/shaders/textured.frag",
                              GL_FRAGMENT_SHADER);
            skyShader->link();
            ...
            // Load the sky texture (note that we don't need mipmaps since we
            // want to avoid any unnecessary blurring while rendering the sky)
            std::string skyTextureFile = config.value<std::string>("sky", "");
            Texture2D* skyTexture =
                texture_utils::loadImage(skyTextureFile, false);
        ...
        // Then we check if there is a postprocessing shader in the
        // configuration
        if (config.contains("postprocess")) {
            glGenFramebuffers(1, &postprocessFrameBuffer);
            glBindFramebuffer(GL_FRAMEBUFFER, postprocessFrameBuffer);

            colorTarget = our::texture_utils::empty(GL_RGBA, this->windowSize,
                                                    GL_UNSIGNED_BYTE);
            depthTarget = our::texture_utils::empty(GL_DEPTH_COMPONENT,
                                                    this->windowSize, GL_FLOAT);

            glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                                   GL_TEXTURE_2D, colorTarget->getOpenGLName(),
                                   0);
            glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
                                   GL_TEXTURE_2D, depthTarget->getOpenGLName(),
                                   0);
            ...
            // Create a sampler to use for sampling the scene texture in the
            ...
            postprocessShader->attach("assets/shaders/fullscreen.vert",
                                      GL_VERTEX_SHADER);
            postprocessShader->attach(
                config.value<std::string>("postprocess", ""),
                GL_FRAGMENT_SHADER);
            ...
            // The default options are fine but we don't need to interact with
            // the depth buffer so it is more performant to disable the depth
            // mask
    ...
    void ForwardRenderer::update(World* world, float deltaTime) {
        ...
        for (auto entity : world->getEntities()) {
            // If we hadn't found a camera yet, we look for a camera in this
            // entity
            if (!camera)
        ...
        for (auto& command : opaqueCommands) {
            ...
            command.material->shader->set(
                "cameraPos", camera->getOwner()->localTransform.position);
            command.mesh->draw();
        }
        ...
        if (this->skyMaterial) {
            ...
            skyMaterial->shader->set("model", skyModelMatrix);
            skyMaterial->shader->set("PV", PV);
            skyMaterial->shader->set("cameraPos", alwaysBehindTransform);
        ...
        if (postprocessMaterial) {
            ...
            postprocessMaterial->shader->set(
                "flashColor", isShooting ? glm::vec3(1.0f, 1.0f, 1.0f)
                                         : glm::vec3(1.0f, 0.0f, 0.0f));
            postprocessMaterial->shader->set(
                "flashIntensity", (isShooting || isDamaged) ? 0.6f : 0.0f);
