...
namespace our {
    ...
    class LightSystem {
    private:
        void setLightingsUniforms(ShaderProgram* shader, Entity* light,
                                  int index) {
            std::string prefix = "lights[" + std::to_string(index) + "].";
            LightComponent* l = light->getComponent<LightComponent>();

            glm::vec3 direction =
                light->getLocalToWorldMatrix() * l->localDirection;
            glm::vec3 position =
                light->getLocalToWorldMatrix() * glm::vec4(0, 0, 0, 1);
            ...
            shader->set(prefix + "cutOff",
                        glm::cos(glm::radians(l->innerCutoff)));
            shader->set(prefix + "outerCutOff",
                        glm::cos(glm::radians(l->outerCutoff)));
        }
    ...
    public:
        void update(World* world, float deltaTime) {
            ...
            auto& meshs =
                world->getEntitiesByTag(MeshRendererComponent::getID());

            unordered_set<ShaderProgram*> uniqueShaders;

            for (auto& mesh : meshs) {
                ShaderProgram* shader =
                    mesh->getComponent<MeshRendererComponent>()
                        ->material->shader;
